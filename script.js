...
function handleHashNavigation() {
    ...
    // --- NEW: INTERCEPT HARDWARE BACK BUTTON FOR IMAGE PREVIEW ---
    const previewModal = document.getElementById('image-preview-modal');
    if (previewModal.classList.contains('active')) {
        // If user presses physical back button while previewing an image, close the image
        previewModal.classList.remove('active');
        // Restore the URL back to #editor so they don't get kicked to the home screen
...
// ==========================================
// 10. MULTIMEDIA & IMAGE PREVIEW (UPLOAD TO BUCKET)
// ==========================================
    ...
    reader.onload = function(event) {
        ...
        img.onload = function() {
            ...
            canvas.toBlob(async (blob) => {
                if(!blob) {
                    alert("Failed to process image data on your device.");
                    return;
                }
    ...
    if(activeNote.images) {
        activeNote.images.forEach((src, i) => {
            ...
            w.querySelector('img').addEventListener('click', () => {
                ...
                // FIX: Open modal visually without altering URL history
...
// FIX: Close the modal visually without touching the history stack
